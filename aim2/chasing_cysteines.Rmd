...
ggplot(data = all_cysteine_range_table, aes(x = max.cysteine_per_window, y = density, color = width)) + geom_line(aes(group=width)) + facet_wrap(~db,ncol=1) + xlim(0,20) + xlab("Maximum number of cysteines") + ylab("Probability density") + labs(colour= "Window size") + theme(strip.text.x = element_text(size=12))
```
