...
names(meta_data3) <- c("Sequence", "Sequence_ID", "Species")
...
ggtree(sp_tree4, aes(color=group, linetype=group)) + geom_tiplab(size=3) + geom_text2(aes(subset = !isTip, label=label), hjust=-.1, size=2) + geom_treescale(offset = 1) + theme(legend.position=c(0.1,0.8), legend.text = element_text(face = "italic"), legend.title = element_blank())
...
# without O. bimaculoides and alignment not trimmed
